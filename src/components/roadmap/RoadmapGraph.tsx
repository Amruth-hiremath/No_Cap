...
export function RoadmapGraph({ concepts, states }: { concepts: Concept[]; states: Record<string, MasteryState> }) {
  ...
  const prereqs = useMemo(() => focus ? focus.prerequisites.map(s => bySlug.get(s)).filter((x): x is Concept => Boolean(x)).slice(-4) : [], [focus, bySlug]);
  const dependents = useMemo(() => focus ? concepts.filter(c => c.prerequisites.includes(focus.slug)).slice(0, 6) : [], [focus, concepts]);
  const related = useMemo(() => focus ? focus.related.map(s => bySlug.get(s)).filter((x): x is Concept => Boolean(x)).slice(0, 6) : [], [focus, bySlug]);
      ...
      {focus && (
        <div className="grid gap-3 rounded-2xl border border-border bg-surface p-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
          ...
          <div className="flex flex-col items-center justify-center px-2 py-2">
            <Link href={`/concepts/${focus.slug}`} className="group w-full min-w-[190px] rounded-xl border-2 border-accent bg-accent-soft p-4 transition-all hover:-translate-y-0.5 hover:shadow-md md:w-[230px]">
      ...
      {related.length > 0 && (
        <div className="rounded-xl border border-border-faint bg-surface-subtle/50 p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {related.map(c => <Link key={c.slug} href={`/concepts/${c.slug}`} className="group inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-2.5 py-1.5 text-xs text-text-secondary transition-all hover:-translate-y-0.5 hover:border-border-strong hover:text-text-primary"><span className="h-1.5 w-1.5 rounded-full" style={{background: stateColor[states[c.slug] ?? 'not_started']}} />{c.title}<ArrowRight className="h-3 w-3 text-text-faint" /></Link>)}
          </div>
        </div>
      )}
    </div>
