...
export function getConceptGraph(slug: string): ConceptGraph {
  ...
  const summary = (s: string): ConceptSummary | undefined => bySlug.get(s);

  return {
    prerequisites: concept.prerequisites
      .map(summary)
      .filter((c): c is ConceptSummary => Boolean(c)),
    ...
    dependents: concepts.filter((c) =>
      c.prerequisites.includes(slug)
    ),
  };
}

export function calculateReadingMinutesLite(concept: {
  summary: string;
  ...
  const text = [
    ...
    ...(concept.blocks ?? []).map((b) => JSON.stringify(b)),
  ...
  const interactive = (concept.blocks ?? []).filter(
    (b: any) =>
      ['quiz', 'scenario', 'simulation', 'mermaid', 'image', 'video'].includes(
        b?.type
      )
  ).length;
  ...
  return Math.max(
    4,
    Math.min(18, Math.round(words / 220 + interactive * 0.22))
  );
}