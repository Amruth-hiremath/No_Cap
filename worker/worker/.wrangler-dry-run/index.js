...
__name(frontendOrigin, "frontendOrigin");
...
function serializeCookie(name, value, request, maxAge, options = {}) {
  const parts = [
    `${name}=${value}`,
    `Max-Age=${maxAge}`,
    "Path=/",
...
function redirectResponse(location, request, env, cookies = []) {
  const headers = corsHeaders(env, request);
...
__name(makeOAuthState, "makeOAuthState");
...
function publicOrigin(request, env) {
  ...
  if (forwarded && /^https:\/\/[^/]+$/i.test(forwarded)) {
    const configured2 = envValue(env, "FRONTEND_ORIGIN");
    if (!configured2 || forwarded.replace(/\/$/, "") === configured2.replace(/\/$/, "")) {
...
async function exchangeOAuthCode(provider, code, redirectUri, clientId, clientSecret) {
  if (provider === "github") {
    ...
    const { response: response2, data: data2 } = await fetchJson(
    ...
    if (!response2.ok) {
      throw new Error(`GitHub token exchange failed (${response2.status})`);
    }
    return data2;
  }
...
__name(githubProfile, "githubProfile");
...
__name(googleProfile, "googleProfile");
async function providerProfile(provider, accessToken) {
  return provider === "github" ? githubProfile(accessToken) : googleProfile(accessToken);
}
__name(providerProfile, "providerProfile");
async function upsertUser(db, profile) {
  const existing = await db.prepare(
    "SELECT id FROM users WHERE auth_provider = ? AND provider_user_id = ?"
  ).bind(profile.provider, profile.provider_id).first();
  if (existing?.id) {
    await db.prepare(
      "UPDATE users SET email = ?, name = ?, avatar_url = ?, updated_at = datetime('now') WHERE id = ?"
    ).bind(profile.email, profile.name, profile.avatar_url, existing.id).run();
    return existing.id;
  }
  if (profile.email && profile.email_verified) {
    const existingByEmail = await db.prepare("SELECT id FROM users WHERE lower(email) = lower(?)").bind(profile.email).first();
...
__name(prefsFromRow, "prefsFromRow");
function profilePayload(row, prefs) {
  return {
    ...
    weekly_minutes: typeof prefs.weekly_minutes === "number" ? prefs.weekly_minutes : 30
  };
}
__name(profilePayload, "profilePayload");
async function getProfileResponse(request, env, userId) {
  const row = await env.DB.prepare(
...
__name(getProfileResponse, "getProfileResponse");
var requestLimiters = /* @__PURE__ */ new Map();
...
async function handleAuthLogin(request, env) {
  ...
  if (!allowRequest(requestFingerprint(request, `auth:${provider}`), 8, 10 * 60 * 1e3)) {
...
async function handleAuthCallback(request, env, provider) {
  const url = new URL(request.url);
  ...
  try {
    ...
    const target = user?.onboarding_completed ? frontendOrigin(env, request) + "/" : frontendOrigin(env, request) + "/onboarding";
    return redirectResponse(
      target,
      request,
      env,
...
async function handleUpdateProfile(request, env) {
  ...
  const current = await env.DB.prepare("SELECT preferences_json FROM users WHERE id = ?").bind(userId).first();
  const prefs = prefsFromRow(current);
  if (Array.isArray(body.goals)) {
    prefs.goals = body.goals.map(String).slice(0, 8).map((value) => value.slice(0, 100));
  }
  if (body.weekly_minutes !== void 0) {
  ...
  const completed = body.onboarding_complete === true || body.completed === true ? 1 : null;
  if (completed !== null) {
    await env.DB.prepare(
      "UPDATE users SET name = ?, timezone = ?, onboarding_completed = ?, preferences_json = ?, updated_at = datetime('now') WHERE id = ?"
    ).bind(name, timezone, completed, JSON.stringify(prefs), userId).run();
  } else {
    await env.DB.prepare(
      "UPDATE users SET name = ?, timezone = ?, preferences_json = ?, updated_at = datetime('now') WHERE id = ?"
    ).bind(name, timezone, JSON.stringify(prefs), userId).run();
  }
  return getProfileResponse(request, env, userId);
}
...
async function handleGetState(request, env) {
  const userId = await getAuthenticatedUserId(request, env);
  if (!userId) return errorResponse("Not authenticated", 401, env, request);
  if (!allowRequest(requestFingerprint(request, `state:${userId}`), 20, 60 * 1e3)) {
    return errorResponse("state_rate_limited", 429, env, request);
  }
  const [masteryRows, reviewRows, attemptRows, noteRows, highlightRows, bookmarkRows, workspaceRows, profile] = await Promise.all([
  ...
  return jsonResponse(
    {
      ...
      confusing_concepts: Array.isArray(prefs.confusing_concepts) ? prefs.confusing_concepts : [],
      streak: prefs.streak ?? {
        current: 0,
        longest: 0,
        last_active: "",
      ...
      last_visited_positions: prefs.last_visited_positions && typeof prefs.last_visited_positions === "object" ? prefs.last_visited_positions : {}
    },
    200,
    env,
    request,
...
__name(syncCollection, "syncCollection");
var syncThrottle = /* @__PURE__ */ new Map();
async function handleSyncState(request, env) {
  ...
  if (throttleNow - last < 5e3) return errorResponse("sync_rate_limited", 429, env, request);
  syncThrottle.set(throttleKey, throttleNow);
  if (syncThrottle.size > 2e3) {
    for (const [k, ts] of syncThrottle) if (throttleNow - ts > 6e4) syncThrottle.delete(k);
  }
  if (!originAllowed(request, env)) {
    return errorResponse("Origin not allowed", 403, env, request);
  }
  ...
  for (const [slug, record] of masteryEntries) {
    const next = {
      learn_score: Number(record?.learn_score ?? 0),
      recall_score: Number(record?.recall_score ?? 0),
      apply_score: Number(record?.apply_score ?? 0),
      explain_score: Number(record?.explain_score ?? 0),
      interview_score: Number(record?.interview_score ?? 0),
      state: String(record?.state ?? "not_started")
    };
  ...
  for (const [slug, item] of reviewEntries) {
    const next = { due_at: String(item?.due_at ?? (/* @__PURE__ */ new Date()).toISOString()), interval_days: Number(item?.interval_days ?? 0.04), ease: Number(item?.ease ?? 2.5), repetitions: Number(item?.repetitions ?? 0), last_quality: item?.last_quality ?? null };
  ...
  const profile = await db.prepare("SELECT preferences_json FROM users WHERE id = ?").bind(userId).first();
  const prefs = prefsFromRow(profile);
  if (Array.isArray(body.confusing_concepts)) {
    prefs.confusing_concepts = body.confusing_concepts;
  }
  if (body.streak !== void 0) prefs.streak = body.streak;
  if (body.last_visited_positions !== void 0) {
  ...
  return jsonResponse(
    { status: "synced", timestamp: (/* @__PURE__ */ new Date()).toISOString() },
    200,
    env,
    request
  );
}
__name(handleSyncState, "handleSyncState");
...
async function handleQuota(request, env) {
  return jsonResponse(
    {
      worker: { used: 0, limit: 1e5, unit: "req" },
      d1_read: { used: 0, limit: 5e6, unit: "rows" },
      d1_write: { used: 0, limit: 1e5, unit: "rows" }
    },
    200,
    env,
    request
...
__name(handleQuota, "handleQuota");
